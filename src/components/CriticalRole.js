...
const CriticalRole = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, 'employment'));
        const fetchedArticles = querySnapshot.docs.map(doc => ({
          title: doc.data().title || doc.id.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase()),
          path: `/employment/${doc.id}`
        }));
  ...
  return (
    <div style={{ marginTop: "2rem" }}>
      <section className="employment-section" style={{ padding: '3rem 2rem', backgroundColor: '#fdfdfd' }}>
